import { lerp, normalize, round } from '$lib/utils/math';

export const calculateDampingRatio = (mass = 1, stiffness, damping) =>
	damping / (2 * Math.sqrt(stiffness * mass));

export const calculateUndampedAngularFrequency = (mass = 1, stiffness) =>
	Math.sqrt(stiffness / mass);

export const calculateAngularFrequency = (undampedAngularFrequency, dampingRatio) =>
	undampedAngularFrequency * Math.sqrt(1 - dampingRatio * dampingRatio);

export const calculatePeakTime = (mass = 1, stiffness, damping) => {
...
export const convertResponseToStiffness = (response, mass = 1) =>
	mass * Math.pow((2 * Math.PI) / response, 2);

export const convertStiffnessToResponse = (stiffness, mass = 1) =>
	(2 * Math.PI) / Math.sqrt(stiffness / mass);

export const convertDampingRatioToDamping = (dampingRatio, stiffness, mass = 1) =>
	dampingRatio * 2 * Math.sqrt(stiffness * mass);

export const convertBounceToDampingRatio = (bounce) => {
...
export const convertDampingRatioToBounce = (dampingRatio) => {
...
const convertFromSpeedBounciness = (speed, bounciness) => {
	...
	return { mass: 1, stiffness, damping, initialVelocity: 0 };
};
...
export const springConverter = {
	...
	'framer-duration': ({ response, bounce }) =>
		convertFromResponseDampingRatio(response, convertBounceToDampingRatio(bounce)),
	'framer-physical': ({ mass, stiffness, damping }) =>
		convertFromMassStiffnessDamping(mass, stiffness, damping),
	'react-spring-friendly': ({ response, dampingRatio }) =>
		convertFromResponseDampingRatio(response, dampingRatio),
	'react-spring-physical': ({ mass, stiffness, damping }) =>
		convertFromMassStiffnessDamping(mass, stiffness, damping),
	'android-springanimation': ({ stiffness, dampingRatio }) =>
		convertFromStiffnessDampingRatio(stiffness, dampingRatio),
	'ios-spring-duration-bounce': ({ response, bounce }) =>
		convertFromResponseDampingRatio(response, convertBounceToDampingRatio(bounce)),
	'ios-spring-response-dampingratio': ({ response, dampingRatio }) =>
		convertFromResponseDampingRatio(response, dampingRatio),
	'ios-spring-mass-stiffness-damping': ({ mass, stiffness, damping }) =>
		convertFromMassStiffnessDamping(mass, stiffness, damping),
	'spring-physical': ({ mass, stiffness, damping, initialVelocity }) => ({
		mass,
		stiffness,
		damping,
		initialVelocity
...
const estimateCriticallyDamped = (
	firstRoot,
	p0,
	v0,
	delta
	...
	const xInflection =
		c1 * Math.exp(r * tInflection) + c2 * tInflection * Math.exp(r * tInflection);
...
export const estimateSpringAnimationDuration = (
	stiffness,
	dampingRatio,
	initialVelocity = 0,
...
export function generateLinearTiming(
	solver,
	duration = 1000,
	tolerance = 0.001
) {
