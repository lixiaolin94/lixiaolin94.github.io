...
function directionVector(dx, dy) {
  const d = dist(dx, dy);
  if (d < DistanceEpsilon) return new Point(0, 0);
...
// Cubic class (cubic Bezier curve)
export class Cubic {
  constructor(...args) {
  ...
  pointOnCurve(t) {
    ...
    return new Point(
      mt3 * this.anchor0X +
        3 * mt2 * t * this.control0X +
        3 * mt * t2 * this.control1X +
        t3 * this.anchor1X,
      mt3 * this.anchor0Y +
        3 * mt2 * t * this.control0Y +
        3 * mt * t2 * this.control1Y +
        t3 * this.anchor1Y
    );
  }
  ...
  split(t) {
    ...
    return [
      new Cubic(q0x, q0y, q1x, q1y, q2x, q2y, q3x, q3y),
      new Cubic(r0x, r0y, r1x, r1y, r2x, r2y, r3x, r3y),
    ];
  }
...
export class Feature {
  ...
  static Corner(cubics, isConvex = true) {
    const corner = new Feature(cubics);
    corner.type = 'Corner';
    corner.convex = isConvex;
    return corner;
  }
...
export class RoundedCorner {
  ...
  getCubics(allowedCut0, allowedCut1 = allowedCut0) {
    ...
    this.center = this.p1.add(
      this.d1.add(this.d2).divide(2).getDirection().multiply(centerDistance)
    );
  ...
  #computeFlankingCurve(
    ...
    const curveStart = corner.add(
      sideDirection.multiply(actualRoundCut * (1 + actualSmoothingValues))
    );

    const p = interpolate(
      circleSegmentIntersection,
      circleSegmentIntersection.add(otherCircleSegmentIntersection).divide(2),
      actualSmoothingValues
    );

    const curveEnd = circleCenter.add(
      directionVector(p.x - circleCenter.x, p.y - circleCenter.y).multiply(actualR)
    );
...
export class RoundedPolygon {
  ...
  static fromVertices(
    ...
    rounding = CornerRounding.Unrounded,
    perVertexRounding = null,
    centerX = Number.MIN_VALUE,
    centerY = Number.MIN_VALUE
  ) {
    ...
    if (perVertexRounding && perVertexRounding.length * 2 !== vertices.length) {
      throw new Error(
        'perVertexRounding list should be either null or ' +
          'the same size as the number of vertices (vertices.length / 2)'
      );
    }
    ...
    for (let ix = 0; ix < n; ix++) {
      ...
      const sideSize = dist(vtxX - nextVtxX, vtxY - nextVtxY);
...
export function verticesFromNumVerts(
  numVertices,
  radius,
  centerX,
  centerY
  ...
  for (let i = 0; i < numVertices; i++) {
    const vertex = radialToCartesian(radius, (Math.PI / numVertices) * 2 * i).add(
