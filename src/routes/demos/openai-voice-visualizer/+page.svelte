<script>
  ...
  onMount(() => {
    ...
    let pane;
    let vertexShaderObj, fragmentShaderObj;

    function createShaderObj(gl, type, source) {
    ...
    function createProgram(gl, vertexSource, fragmentSource) {
      vertexShaderObj = createShaderObj(gl, gl.VERTEX_SHADER, vertexSource);
      fragmentShaderObj = createShaderObj(gl, gl.FRAGMENT_SHADER, fragmentSource);

      const prog = gl.createProgram();
      gl.attachShader(prog, vertexShaderObj);
      gl.attachShader(prog, fragmentShaderObj);
      gl.linkProgram(prog);
    ...
    function processAudioData(prevAudioData, prevCumulativeAudioData, deltaTimeS, audioDataRaw) {
      const audioDataIncrement = audioDataRaw.map(
        (v) => v * deltaTimeS * AUDIO_UPDATE_INTERVAL * AUDIO_TIME_CONSTANT_MULTIPLIER
      );

      const audioAlpha = smoothTransition(deltaTimeS, AUDIO_TIME_CONSTANT_DELTA);

      const newAudioData = prevAudioData.map((prev, i) =>
        lerp(prev, prev + audioDataIncrement[i], audioAlpha)
      );

      const cumulativeIncrement = audioDataRaw.map(
        (v) => v * deltaTimeS * AUDIO_UPDATE_INTERVAL * AUDIO_CUMULATIVE_MULTIPLIER
      );
      ...
      const newCumulativeAudioData = prevCumulativeAudioData.map((prev, i) =>
        lerp(prev, rawCumulativeAudio[i], cumulativeAlpha)
      );
    ...
    function updateAudioData() {
      ...
      const bandDataResult = calculateBandMagnitudes(normalizedFreqData, {
        bandCount: 4,
        binCount: 1,
      ...
      const processedData = processAudioData(audioData, cumulativeAudioData, deltaTime, bandDataResult);
    ...
    async function init() {
      try {
        ...
        generalFolder.addBinding(config, 'mic').on('change', (event) => {
          if (event.value) {
            startMicrophone();
          } else {
            stopMicrophone();
        ...
        generalFolder.addBinding(config, 'advanced');

        colorFolder.addBinding(config, 'main', {
          color: { type: 'float' }
        });
        colorFolder.addBinding(config, 'low', {
          color: { type: 'float' }
        });
        colorFolder.addBinding(config, 'mid', {
          color: { type: 'float' }
        });
        colorFolder.addBinding(config, 'high', {
          color: { type: 'float' }
        });
    ...
    return () => {
      ...
      if (gl) {
        ...
        if (glUniformBuffer) {
          gl.deleteBuffer(glUniformBuffer);
        }
        if (vao) {
        ...
        if (program) {
          ...
          if (fragmentShaderObj) {
            gl.detachShader(program, fragmentShaderObj);
...
<svelte:head>
  <title>OpenAI Voice Visualizer</title>
</svelte:head>
